...
module.exports = {
  ...
  update: [
    ...
    check('*.fields.longitdute').optional().isFloat().withMessage('must be integer above 0'),
    check('*.fields.latitude').optional().isFloat().withMessage('must be integer above 0'),
  ],
  create: [
    // check('*.tags.deviceid').optional().isInt({min: 1}).withMessage('must be integer above 0'),
    check('*.tags.device').optional().isLength({ min: 1 }).withMessage('must be at least 1 chars long'),
